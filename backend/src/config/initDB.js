...
async function initializeDB() {
  ...
  const cleanupCondition = `
    WHERE driver_id NOT IN (SELECT id FROM users WHERE email IN ('claudemir', 'jonathan'))
    OR (driver_id = (SELECT id FROM users WHERE email = 'claudemir') AND created_at < CURRENT_DATE - INTERVAL '2 days')
  `;
  
  const handoverCleanupCondition = `
    WHERE driver_id NOT IN (SELECT id FROM users WHERE email IN ('claudemir', 'jonathan'))
    OR (driver_id = (SELECT id FROM users WHERE email = 'claudemir') AND checkout_time < CURRENT_DATE - INTERVAL '2 days')
  `;
